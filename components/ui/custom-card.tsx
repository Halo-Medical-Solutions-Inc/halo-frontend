  ...
  while (currentIndex < text.length) {
    ...
    switch (earliestMatch.type) {
      ...
      case 'italic':
        ...
        break;
      case 'underline':
        tokens.push(<span key={key} style={{ textDecoration: 'underline' }}>{innerContent}</span>);
        break;
    }
...
const CustomCard = ({ step, currentStep, totalSteps, nextStep, prevStep, skipTour, arrow }: CustomCardProps) => {
  ...
  const formattedContent = typeof step.content === 'string' 
    ? parseFormattedText(step.content) 
    : step.content;
